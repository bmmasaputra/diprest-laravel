...
<body class="bg-gray-100 min-h-screen">
    ...
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA PENELITIAN</h2>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="relative w-full max-w-xs ml-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari data penelitian...">
                    </div>
                </div>
            </div>
            ...
            <div id="table-container">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                        @forelse($datapenelitian as $item)
                        <tr>
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $datapenelitian->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $datapenelitian->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $datapenelitian->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($datapenelitian->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $datapenelitian->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($datapenelitian->getUrlRange(1, $datapenelitian->lastPage()) as $page => $url)
                        @if ($page == $datapenelitian->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($datapenelitian->hasMorePages())
                        <li>
                            <a href="{{ $datapenelitian->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datapenelitian.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
