...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Proyek Kemanusiaan UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA PROYEK KEMANUSIAAN</h2>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="relative w-full max-w-xs ml-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari data proyek kemanusiaan...">
                    </div>
                </div>
            </div>
            ...
            <div id="table-container">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                        @forelse($datakemanusiaan as $item)
                        <tr>
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $datakemanusiaan->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $datakemanusiaan->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $datakemanusiaan->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($datakemanusiaan->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $datakemanusiaan->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($datakemanusiaan->getUrlRange(1, $datakemanusiaan->lastPage()) as $page => $url)
                        @if ($page == $datakemanusiaan->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($datakemanusiaan->hasMorePages())
                        <li>
                            <a href="{{ $datakemanusiaan->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datakemanusiaan.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
