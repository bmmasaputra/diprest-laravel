...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Prestasi UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA MAGANG</h2>
    </div>
            ...
            <div id="table-container">
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $datamagang->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $datamagang->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $datamagang->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($datamagang->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $datamagang->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($datamagang->getUrlRange(1, $datamagang->lastPage()) as $page => $url)
                        @if ($page == $datamagang->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($datamagang->hasMorePages())
                        <li>
                            <a href="{{ $datamagang->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datamagang.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
