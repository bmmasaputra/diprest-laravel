...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Studi Proyek Independen UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA STUDI PROYEK INDEPENDEN</h2>
    </div>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div id="table-container">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                        @forelse($dataindependen as $item)
                        <tr>
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $dataindependen->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $dataindependen->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $dataindependen->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($dataindependen->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $dataindependen->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($dataindependen->getUrlRange(1, $dataindependen->lastPage()) as $page => $url)
                        @if ($page == $dataindependen->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($dataindependen->hasMorePages())
                        <li>
                            <a href="{{ $dataindependen->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('dataindependen.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
