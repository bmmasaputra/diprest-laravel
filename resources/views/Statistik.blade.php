    ...
    <div class="relative isolate overflow-hidden bg-gray-900 min-h-screen flex items-center py-24 sm:py-32">
        ...
        <div class="mx-auto max-w-7xl px-6 lg:px-8 w-full">
            <div class="mx-auto max-w-2xl lg:mx-0">
                ...
                <p class="mt-6 text-lg leading-8 text-white">Berikut adalah statistik prestasi mahasiswa Universitas Palangka Raya yang telah diraih selama ini.</p>
            </div>
                ...
                <div class="grid justify-center items-center grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8">
                    ...
                    <a href=" @if($card['title'] === 'Data Prestasi') {{ url('/data-prestasi') }} @elseif($card['title'] === 'Data Organisasi') {{ url('/data-organisasi') }} @elseif($card['title'] === 'Data Pertukaran Mahasiswa') {{ url('/data-pertukaran-mahasiswa') }} @elseif($card['title'] === 'Data Magang') {{ url('/data-magang') }} @elseif($card['title'] === 'Data Mengajar') {{ url('/data-mengajar') }} @elseif($card['title'] === 'Data Penelitian') {{ url('/data-penelitian') }} @elseif($card['title'] === 'Data Proyek Kemanusiaan') {{ url('/data-proyek-kemanusiaan') }} @elseif($card['title'] === 'Data Proyek') {{ url('/data-proyek') }} @elseif($card['title'] === 'Data Wirausaha') {{ url('/data-wirausaha') }} @elseif($card['title'] === 'Data Studi Proyek Independen') {{ url('/data-studi-proyek-independen') }} @elseif($card['title'] === 'Data Pengabdian') {{ url('/data-pengabdian') }} @elseif($card['title'] === 'Data Rekognisi') {{ url('/data-rekognisi') }} @elseif($card['title'] === 'Data Pembinaan Mental Kebangsaan') {{ url('/data-pembinaan-mental-kebangsaan') }} @elseif($card['title'] === 'Data Sertifikasi') {{ url('/data-sertifikasi') }} @endif" class="flex flex-col items-center bg-white border border-gray-200 rounded-lg shadow-sm max-w-sm w-full min-h-[300px] h-[300px] mx-auto hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 transform transition-transform duration-500 hover:scale-105">
                        ...
                        <div class="flex flex-col justify-between p-4 leading-normal w-full flex-1">
                            <h5 class="mb-2 text-2xl font-bold tracking-tight text-gray-900 dark:text-white text-center">{{ $card['title'] }}</h5>
                            <p class="mb-3 font-normal text-gray-700 dark:text-gray-400 text-center flex-1 text-lg">{{ $card['deskripsi'] ?? '' }}</p>
                        </div>
                    </a>
                    @endforeach
                </div>
