...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Wirausaha UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA WIRAUSAHA</h2>
    </div>
                ...
                <div class="relative w-full max-w-xs ml-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari data wirausaha...">
                    </div>
                </div>
            </div>
            ...
            <div id="table-container">
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $datawirausaha->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $datawirausaha->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $datawirausaha->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($datawirausaha->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $datawirausaha->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($datawirausaha->getUrlRange(1, $datawirausaha->lastPage()) as $page => $url)
                        @if ($page == $datawirausaha->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($datawirausaha->hasMorePages())
                        <li>
                            <a href="{{ $datawirausaha->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datawirausaha.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
