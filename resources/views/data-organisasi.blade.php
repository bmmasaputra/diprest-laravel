...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Organisasi UPR"></x-Navbar>

    <!-- Header Section -->
    <div class="pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA ORGANISASI</h2>
    </div>
    ...
    <div class="container mx-auto px-4 py-10">
        <div class="bg-white rounded-lg shadow-md p-6">
            ...
            <div class="flex items-center justify-between mb-6">
                <!-- Button Kembali -->
                <a href="/statistik"
                    class="text-white bg-gray-700 hover:bg-red-800 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-5 py-2.5 inline-flex items-center">
            ...
            <div id="table-container">
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $dataorganisasi->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $dataorganisasi->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $dataorganisasi->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($dataorganisasi->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $dataorganisasi->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($dataorganisasi->getUrlRange(1, $dataorganisasi->lastPage()) as $page => $url)
                        @if ($page == $dataorganisasi->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($dataorganisasi->hasMorePages())
                        <li>
                            <a href="{{ $dataorganisasi->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('dataorganisasi.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
