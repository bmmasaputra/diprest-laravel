...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-white" title="Data Pengabdian UPR"></x-Navbar>
        ...
        <div class="bg-white rounded-lg shadow-md p-6">
                ...
                <div class="relative w-full max-w-xs ml-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari data pengabdian...">
                    </div>
                </div>
            </div>
            ...
            <div id="table-container">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    ...
                    <tbody>
                        @forelse($dataPengabdian as $item)
                        <tr>
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $dataPengabdian->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $dataPengabdian->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $dataPengabdian->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($dataPengabdian->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $dataPengabdian->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($dataPengabdian->getUrlRange(1, $dataPengabdian->lastPage()) as $page => $url)
                        @if ($page == $dataPengabdian->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($dataPengabdian->hasMorePages())
                        <li>
                            <a href="{{ $dataPengabdian->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datapengabdian.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
