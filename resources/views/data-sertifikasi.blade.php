...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Sertifikasi UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA SERTIFIKASI</h2>
    </div>
                ...
                <div class="relative w-full max-w-xs ml-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari data sertifikasi...">
                    </div>
                </div>
            </div>
            ...
            <div id="table-container">
                ...
                <table class="w-full border-collapse border border-gray-300">
                    <thead class="bg-gray-100">
                        <tr>
                            ...
                            <th class="border px-4 py-2">Dosen Pendamping</th>
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td class="border px-4 py-2">{{ $item->nama_skema_sertifikasi }}</td>
                            <td class="border px-4 py-2">{{ $item->tingkat_kegiatan }}</td>
                            <td class="border px-4 py-2">{{ $item->tahun_kegiatan }}</td>
                            <td class="border px-4 py-2">{{ $item->dosen_pendamping }}</td>
                        </tr>
                        @empty
                        <tr>
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $datasertifikasi->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $datasertifikasi->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $datasertifikasi->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($datasertifikasi->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $datasertifikasi->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($datasertifikasi->getUrlRange(1, $datasertifikasi->lastPage()) as $page => $url)
                        @if ($page == $datasertifikasi->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($datasertifikasi->hasMorePages())
                        <li>
                            <a href="{{ $datasertifikasi->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datasertifikasi.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
