...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Mengajar UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA MENGAJAR</h2>
    </div>
                ...
                <div class="relative w-full max-w-xs ml-4">
                    ...
                    <div class="relative">
                        ...
                        <input type="text" id="table-search" value="{{ request('search') }}"
                            ...
                            placeholder="Cari data mengajar...">
                    </div>
                </div>
            </div>
            ...
            <div id="table-container">
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $datamengajar->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $datamengajar->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $datamengajar->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($datamengajar->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $datamengajar->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($datamengajar->getUrlRange(1, $datamengajar->lastPage()) as $page => $url)
                        @if ($page == $datamengajar->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($datamengajar->hasMorePages())
                        <li>
                            <a href="{{ $datamengajar->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datamengajar.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
