...
<body class="bg-gray-100 min-h-screen">
    <x-Navbar background="bg-gray-900" title="Data Pembinaan Mental Kebangsaan UPR"></x-Navbar>

    <!-- Header Section -->
    <div class=" pt-28 pb-10">
        <h2 class="text-gray-900 text-3xl font-semibold text-center tracking-tight">DATA PEMBINAAN MENTAL KEBANGSAAN</h2>
    </div>
            ...
            <div id="table-container">
                ...
                <nav class="flex items-center justify-between pt-4" aria-label="Table navigation">
                    <span class="text-sm font-normal text-gray-500">
                        Menampilkan
                        <span class="font-semibold text-gray-900">{{ $dataPembinaan->firstItem() }}</span> -
                        <span class="font-semibold text-gray-900">{{ $dataPembinaan->lastItem() }}</span>
                        dari <span class="font-semibold text-gray-900">{{ $dataPembinaan->total() }}</span> data
                    </span>

                    <ul class="inline-flex -space-x-px text-sm h-8">
                        {{-- Tombol Previous --}}
                        @if ($dataPembinaan->onFirstPage())
                        <li>
                            <span
                        ...
                        @else
                        <li>
                            <a href="{{ $dataPembinaan->previousPageUrl() }}"
                        ...
                        {{-- Nomor Halaman --}}
                        @foreach ($dataPembinaan->getUrlRange(1, $dataPembinaan->lastPage()) as $page => $url)
                        @if ($page == $dataPembinaan->currentPage())
                        <li>
                        ...
                        {{-- Tombol Next --}}
                        @if ($dataPembinaan->hasMorePages())
                        <li>
                            <a href="{{ $dataPembinaan->nextPageUrl() }}"
...
<script>
    ...
    searchInput.addEventListener('keyup', function() {
        ...
        timeout = setTimeout(() => {
            fetch(`{{ route('datapembinaan.index') }}?search=${encodeURIComponent(this.value)}`)
                .then(res => res.text())
                .then(html => {
